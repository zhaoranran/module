...
JC.SideMenu = Clazz.extend({
    ...
    getTemplateLeft0: function(menus, levels) {
        ...
        result.push('<li id="topnav-other_modules" style="display: none;"><a href="#"><i class="topnav-other_modules fa-column-selection"></i>其他模块</a></li>');
        ...
        if (menus && menus.children) {
            ...
            for (var i = 0; i < menus.children.length; i++) {
                ...
                if (menu.children) {
                    ...
                    result.push('<span class="pull-right"><i class="fa fa-chevron-up text-active"></i><i class="fa fa-chevron-down text" style="display: inline-block"></i></span>');
                }
                result.push('<span>' + menu.name + '</span>');
                result.push('</a>');
                if (menu.children) {
    ...
    setData: function(data, callback) {
        var data = $.isArray(data) ? data : $.parseJSON(data);
        this.cacheData = this._parse(data);
        this.level1 = data;
