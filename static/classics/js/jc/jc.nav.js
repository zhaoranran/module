...
//有声无声
function turnVoice2(obj) {
    if ($(obj).attr("name") == "on") {
        $.cookie('voiceSwitch', 'true');
    } else {
        $.cookie('voiceSwitch', 'false');
    }
    $(obj).addClass("active off").removeClass("on");
    $(obj).siblings().removeClass("active off").addClass("on");
...
$(document).on('click', '[data-id^="class"]', function(e) {
    ...
    //$this.hasClass('active')?($this.find('i').attr({"data-original-title":"隐藏菜单","class":"fa fa-caret-left"})):($this.find('i').attr({"data-original-title":"显示菜单","class":"fa fa-caret-right"}));
    //$this.hasClass('active')?($this.find('i').attr({"data-original-title":"隐藏菜单","class":"fa fa-caret-left"})):($this.find('i').attr({"data-original-title":"显示菜单","class":"fa fa-caret-right"}));
