...
JC.SideMenu = Clazz.extend({
    ...
    _addEvent: function() {
        ...
        this.$ele.on('mouseout', ' a', function(e) {
    ...
    getTemplateLeft0: function(menus, levels) {
        //if (this.headMenu) JC.navigation(this.headMenu.id);
            ...
            for (var i = 0; i < menus.length; i++) {
                ...
                result.push('<li ' + ac + ' li-title="' + menu.name + '" data-toggle="tooltip" data-placement="right" title="" data-original-title=""><a href="javascript:void(0)" ' + ac + ' name="' + menu.name + '" data-head="menu" id="jcleftmenu_' + menu.id + '" action="' + menu.action + '" ' + this.getEventDom(menu) + '>');


                result.push('<i class="' + menu.icon + '"><b class="b-bg-' + (i % 8) + '"></b></i>');
                if (menu.children) {
                    //result.push('<span class="pull-right"></span>');
...
//点击退出全屏事件/*2018/6/24chenmj修改*/
    $(document).on('click', '.bigSmall >.active', function () {
...
/*
//暂存
var navXsExists = $("aside.aside-md").hasClass("nav-xs");
if(navXsExists){
    $("aside.aside-md").find(".user-avatar .h-nav-list").css("left","43px");
}else {
    $("aside.aside-md").find(".user-avatar .h-nav-list").css("left","199px");
}*/
